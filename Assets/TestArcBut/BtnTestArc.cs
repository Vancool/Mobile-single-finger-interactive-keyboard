...
public class BtnTestArc : MonoBehaviour
{
    //public Text text;
    ...
    public void changeLayout(float max_angle  , float blankRate, float btnLen ,float r,float off_set_x,float off_set_y,float alpha)
    {
        ...
        */
        ...
        for (int i = 0; i < btnsSize; i++)
        {
            float arc_angle = (float)( (DisplaySpin.off_spin_angle_in_degree+off_angle + delt_angle * i) / 180 * 3.1415926);
            ...
            rt.Rotate(new Vector3(0, 0, 1), off_angle+ delt_angle * i+ DisplaySpin.off_spin_angle_in_degree);
        }
